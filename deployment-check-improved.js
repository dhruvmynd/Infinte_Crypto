// Improved script to check the deployment status and configuration
...
// Define the Stripe endpoints to check
const STRIPE_ENDPOINTS = [
  { 
    path: '/api/create-checkout-session', 
    method: 'POST',
    body: { packId: 'basic', packType: 'words', userId: 'test-user-id' },
    description: 'Stripe checkout endpoint' 
  },
  { 
    path: '/.netlify/functions/api/create-checkout-session', 
    method: 'POST',
    body: { packId: 'basic', packType: 'words', userId: 'test-user-id' },
    description: 'Direct Stripe checkout endpoint' 
  }
];
...
async function checkStripeEndpoint(baseUrl, endpoint) {
  const url = `${baseUrl}${endpoint.path}`;
  console.log(`\n🔍 Checking Stripe endpoint ${url}...`);
    ...
    try {
      ...
      if (contentType.includes('application/json')) {
        try {
          ...
          // Check if we have a session ID and URL for Stripe
          if (json.id && json.url) {
      ...
      let isSuccess = status >= 200 && status < 300 && contentType.includes('application/json');
...
async function main() {
  ...
  for (const endpoint of STRIPE_ENDPOINTS) {
    const result = await checkStripeEndpoint(siteUrl, endpoint);
  ...
  const stripeSuccessCount = stripeResults.filter(r => r.isSuccess).length;
  console.log(`💳 ${stripeSuccessCount}/${STRIPE_ENDPOINTS.length} Stripe endpoints are working correctly.`);
  ...
  if (successCount === ENDPOINTS.length && stripeSuccessCount === STRIPE_ENDPOINTS.length) {
    ...
    if (stripeSuccessCount < STRIPE_ENDPOINTS.length) {
      console.log('7. Check your Stripe configuration in the serverless function');
      console.log('8. Ensure the Stripe API keys are correctly set in your environment variables');
    }
  }
}
