...
export function CheckoutModal({ isOpen, onClose, type }: CheckoutModalProps) {
  ...
  const handleExternalCheckout = async (packageId: string) => {
    if (!isLoggedIn) {
      setError('You must be logged in to make a purchase');
      return;
    }
  ...
  return (
    <>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleExternalCheckout(pkg.id)}
                          disabled={loading || !isLoggedIn}
                          className="flex items-center gap-2 px-4 py-2 bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-500 text-gray-800 dark:text-white rounded-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        >
      ...
      {showCheckoutForm && selectedPackage && (
        <StripeCheckoutForm 
          isOpen={showCheckoutForm}
          onClose={() => setShowCheckoutForm(false)}
          selectedWords={[]}
          onSuccess={handleCheckoutSuccess}
          tokenPackage={packages.find(pkg => pkg.id === selectedPackage)}
        />
      )}
    </>
  );
